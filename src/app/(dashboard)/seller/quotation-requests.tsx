...
const ServiceIconWrapper = ({ serviceName }: { serviceName: string }) => {
  ...
  const isLeak = serviceName.toLowerCase().includes('leak') || serviceName.toLowerCase().includes('pipe');
  ...
  useEffect(() => {
    const loadRequests = async () => {
      ...
      const formatted = stored.map((r: any) => ({
        id: r.requestId,
        title: r.items?.some((i: any) => i.name === 'Copper Pipe') ? 'AC Repair' : 'Material Request',
        serviceName: r.items?.some((i: any) => i.name === 'Copper Pipe') ? 'AC Repair' : 'Material Request',
      ...
      const fallbacks = [
        { id: 'RQ123456', title: 'AC Repair', serviceName: 'AC Repair', customer: 'James Wilson', bookingId: 'BK-9821', date: 'Oct 24, 2023', status: 'pending_quote', estimate: '$120.00', requestId: 'RQ123456' },
        { id: 'RQ882312', title: 'Pipe Leakage', serviceName: 'Pipe Leakage', customer: 'Sarah Connor', bookingId: 'BK-5512', date: 'Oct 23, 2023', status: 'pending_quote', estimate: '$85.00', requestId: 'RQ882312' },
        { id: 'RQ773211', title: 'Wiring Installation', serviceName: 'Wiring Installation', customer: 'Michael Scott', bookingId: 'BK-1029', date: 'Oct 22, 2023', status: 'pending_quote', estimate: '$210.00', requestId: 'RQ773211' },
      ];
    ...
    <GradientBackground style={styles.container}>
      <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
        ...
        <View style={styles.header}>
          <TouchableOpacity onPress={() => router.replace('/(dashboard)')} style={styles.menuButton}>
          ...
          {filtered.map((req) => (
            <View key={req.id} style={styles.card}>
              <View style={styles.cardHeader}>
                ...
                <View style={[
                  styles.badge, 
                  ['pending_quote', 'quoted'].includes(req.status) && { backgroundColor: '#EFF6FF' },
                  req.status === 'approved' && { backgroundColor: '#ECFDF5' },
                  req.status === 'completed' && { backgroundColor: '#F0FDF4' },
                  req.status === 'rejected' && { backgroundColor: '#FEF2F2' }
                ]}>
                  <Text style={[
                    styles.badgeText,
                    ['pending_quote', 'quoted'].includes(req.status) && { color: '#2563EB' },
                    req.status === 'approved' && { color: '#10B981' },
                    req.status === 'completed' && { color: '#166534' },
                    req.status === 'rejected' && { color: '#EF4444' }
                  ]}>
                    {req.status === 'pending_quote' ? 'Pending' : req.status === 'quoted' ? 'Quoted' : req.status.toUpperCase()}
                  </Text>
                </View>
              </View>
              ...
              <View style={styles.detailsGrid}>
                <View style={styles.detailsCol}>
                  ...
                  <Text style={[styles.gridLabel, { marginTop: 12 }]}>REQUESTED ON</Text>
                ...
                <View style={styles.detailsCol}>
                  ...
                  <Text style={[styles.gridLabel, { marginTop: 12 }]}>QUOTE ESTIMATE</Text>
              ...
              >
                <Text style={styles.viewDetailsBtnText}>View Request Details</Text>
...
const styles = StyleSheet.create({
  ...
  detailsCol: { flex: 1 },
  gridLabel: { fontSize: 9, fontWeight: '700', color: '#94A3B8', letterSpacing: 0.5 },
  gridValue: { fontSize: 12, fontWeight: '600', color: '#334155', marginTop: 4 },
  gridValueEstimate: { fontSize: 12, fontWeight: '700', color: '#16A34A', marginTop: 4 },
